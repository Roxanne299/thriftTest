...
class Iface(object):
    ...
    def remove_user(self, suer, info):
        """
        Parameters:
         - suer
         - info
...
class Client(Iface):
    ...
    def remove_user(self, suer, info):
        ...
        """
        self.send_remove_user(suer, info)
        return self.recv_remove_user()

    def send_remove_user(self, suer, info):
...
class Processor(Iface, TProcessor):
    ...
    def process_remove_user(self, seqid, iprot, oprot):
        ...
        result = remove_user_result()
        try:
            result.success = self._handler.remove_user(args.suer, args.info)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
...
class remove_user_args(object):
    ...
    def __init__(self, suer=None, info=None,):
        self.suer = suer
        self.info = info
    ...
    def write(self, oprot):
        ...
        if self.suer is not None:
            oprot.writeFieldBegin('suer', TType.STRUCT, 1)
            self.suer.write(oprot)
            oprot.writeFieldEnd()
        if self.info is not None:
            oprot.writeFieldBegin('info', TType.STRING, 2)
            oprot.writeString(self.info.encode('utf-8') if sys.version_info[0] == 2 else self.info)
...
remove_user_args.thrift_spec = (
    None,  # 0
    (1, TType.STRUCT, 'suer', [User, None], None, ),  # 1
    (2, TType.STRING, 'info', 'UTF8', None, ),  # 2
)
